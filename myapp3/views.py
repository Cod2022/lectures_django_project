...
from django.shortcuts import render, get_object_or_404 # эта функция позволяет обратиться к базе данных и получить оттуда объект
...
def my_view(request):
    context = {"name": "John"} 
    return render(request, "myapp3/my_template.html", context) # каталог templates не передаём в путь, его Django находит сам
