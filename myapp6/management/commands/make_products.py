...
class Command(BaseCommand): 
    ...
    def handle(self, *args, **kwargs): 
        ...
        Product.objects.bulk_create(products) # bulk_create добавляет сразу список объектов в базу данных (по одному). Вместо того, чтобы использовать Obj.save()
                                              # добавлять в базу сразу списком быстрее, чем по одному через save()